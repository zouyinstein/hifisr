...
cat mito_commands.txt | while read i; do echo "sh " $i >> mito_commands.list; done
cat plastid_commands.txt | while read i; do echo "sh " $i >> plastid_commands.list; done
ParaFly -c cat mito_commands.list -CPU ${thread} -failed_cmds mito_commands.list.failed
ParaFly -c cat plastid_commands.list -CPU ${thread} -failed_cmds plastid_commands.list.failed
